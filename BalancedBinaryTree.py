...
def is_balanced(tree_root):
    ...
    # We short-circut as soon as we find more than 2
    depths = []
    ...
    while len(nodes):
        ...
        # Cases: we found a leaf
        if (not node.left) and (not node.right):
            # care only about it if it's a new depth
            if depth not in depths:
                depths.append(depth)

                # unbalanced tree if 1) more than 2 different leaf depths, 2) 2 leaf depths that are more than 1 apart
                if (len(depths) < 2) or (len(depths) == 2 and abs(depths[0] - depths[1]) > 1):
                    return False

        else:
            # Case: this isn't a leaf - keep stepping through
